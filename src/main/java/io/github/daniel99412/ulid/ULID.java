...
public class ULID implements Serializable, Comparable<ULID> {
            ...
            private static ULID last = fromInstant(Instant.EPOCH);
            ...
            /**
             * @deprecated Use {@link #fromTimestamp(Instant)} instead.
            ...
            public static ULID fromInstant(Instant instant) {
                ...
                long randomPart = RANDOM.nextLong() & Long.MAX_VALUE;
             ...
             * Generates a ULID from a given {@link Instant}.
             *
             * @param timestamp The timestamp to use for the ULID.
             * @return A new ULID instance.
             */
            public static ULID fromTimestamp(Instant timestamp) {
                long timestampMilli = timestamp.toEpochMilli();
                long randomPart = RANDOM.nextLong() & Long.MAX_VALUE;
                return new ULID(timestampMilli, randomPart);
            }
            ...
            public static synchronized ULID nextULID() {
                long timestamp = Instant.now().toEpochMilli();
                if (timestamp == last.getMostSignificantBits()) {
                    last = last.increment();
                } else {
                    long randomPart = RANDOM.nextLong() & Long.MAX_VALUE;
                    last = new ULID(timestamp, randomPart);
                }
                return last;
            }
